"""
GENDATA FOR WAVE INTERATIONS PROJECT (025_05 , Seperation by 2 ridges, Fr 0.84)
1 December 2024
Klema, Matthew
...
u0 = 0.0251                     # open-boundary inflow velocity (m/s)
...
h = (h0 / 4) * (1 + np.cos((2 * np.pi * (x - 1.5 * lambda_x)) / (2 * L) - np.pi / 2))


# Adjusting the height of the second curve to be half the height of the first
h_second_curve = (h0 / 2) * (1 + np.cos((2 * np.pi * ((x-50) - 0.5 * lambda_x)) / (2 * L) - np.pi / 2))
...
for i in range(len(x)):
    if x[i] < (151.656):
        H[i] = -H0
for i in range(len(x)):
    if x[i] > (177.656) and x[i] < (226.35):
        H[i] = -H0
for i in range(len(x)):
    if x[i] > (226.35) and x[i] < (252.3):
...
with open('README.txt', 'w') as fid:
    ...
    fid.write("Simulation time step set at value 'deltaT = 0.1s'\n")
